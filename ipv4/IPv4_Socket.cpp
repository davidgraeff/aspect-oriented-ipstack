...
void IPv4_Socket::setupHeader(IPv4_Packet* packet, unsigned datasize, UInt8 protocol_id){
  packet->set_ihl(IPV4_MIN_HEADER_SIZE/4);
  packet->set_version(IPV4_VERSION);
  packet->set_tos(IPV4_DEFAULT_TOS);
  packet->set_total_len(datasize + IPV4_MIN_HEADER_SIZE);
  packet->set_id(id++);
  packet->set_flags(IPV4_DEFAULT_FLAGS);
  packet->set_fragment_offset(IPV4_NO_FRAGMENT_OFFSET);
  packet->set_ttl(IPV4_DEFAULT_TTL);
...
void IPv4_Socket::send(void* data, unsigned datasize, UInt8 protocol_id){
  ...
  IPv4_Packet* packet = (IPv4_Packet*) (((UInt8*)data) - IPV4_MIN_HEADER_SIZE);
  setupHeader(packet, datasize, protocol_id);

  //Ethernet aspect affecting this:
  interface->send(packet, datasize + IPV4_MIN_HEADER_SIZE);
}
...
unsigned IPv4_Socket::getMTU(){
  if(interface != 0){
    ...
    return (interface->getMTU() - IPV4_MIN_HEADER_SIZE);
  }
  else{
    return 0;
  }
